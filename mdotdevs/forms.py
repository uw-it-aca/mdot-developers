...
class ReviewForm(forms.Form):
    ...
    support_contact = forms.CharField(widget=forms.widgets.Textarea(),label='Other contact information', required=False)
    ...
    app_documentation = forms.URLField(label='Provide URL for app documentation', required=False)  
    app_code = forms.CharField(widget=forms.widgets.Textarea(),label='How might we be able to review the app?')
    anything_else = forms.CharField(widget=forms.widgets.Textarea(),label='Anything else that we should know?', required=False)
